...
class AssistantAgentChatReq(BaseModel):
    """辅助 Agent 聊天请求。单会话 + 全局内置 app，故不带 conversation_id / app_id。"""
    ...
    provider: Optional[str] = Field(
        ...
        description="本轮覆盖的模型 provider；缺省回退 app/env 默认。provider 与 model 须成对传。",
    )
    model: Optional[str] = Field(
        default=None, max_length=128,
        description="本轮覆盖的模型名；须与 provider 成对传，且经注册表校验合法。",
    )
