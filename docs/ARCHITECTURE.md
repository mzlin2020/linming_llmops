...
- **图像生成（v1.1）**：`LanguageModelManager.generate_images`（OpenAI 兼容 `/images/generations`）+ `image_generation_service`/`image_generation_handler`/`/images*` 路由（文生图 / 图生图 / 历史画廊）。上游返回的图片下载落 `StorageService` 后经「能力 URL」`/api/images/file/<uuid>.<ext>` 对外（无鉴权、不可猜，供 `<img>` 加载）；并提供 `image_generation` 内置工具（`admin_only=false`），让带工具的 agent 在对话里出图（markdown 渲染）。`ai_image` 表已在迁移基线内，**零新表**。
