...
| Phase 8 | v1.1：工作流 + 图像 + TTS | 暴露工作流引擎 + 可视化编辑器；图像/TTS 供应商可插拔 | ⏳ 计划中 |
...
- **工作流可视化编辑器**：暴露 `core/workflow` 引擎的服务/处理器 + 前端图编辑器（建图 → 调试运行）。
- **图像生成**：供应商可插拔实现，默认关闭，未配置时优雅降级。
- **TTS（文本转语音）**：供应商可插拔实现，默认关闭。
